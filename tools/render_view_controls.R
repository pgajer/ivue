...
widgets[[3]] <- plot3D.groups(X,c(a='A',b='B',c='C'),limits=limits,
    camera=camera.zup(elevation=35,turn=0,fov=45),height=420,point.size=15,
    description='Perspective reference observations')
...
types <- unique(as.character(readRDS(system.file('extdata','retinal-development.rds',
    package='ivue'))$annotations$cell.type))
stopifnot(length(types)==11L)
long <- plot3D.groups(cbind(seq_along(types),0,0),types,
    legend.title='Annotated retinal cell types across the display cohort', 
