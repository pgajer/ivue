...
installed `rgl` package; color scales and mapping do not. Install the backend
with `install.packages("rgl")`. Optional graph layouts and igraph inputs require
...
precision; `digits` affects legend text only. There is no automatic
...
color per value. Do not supply both.
...
plot3D.cltrs(X, groups, scale = group.scale, point.size = 5,
...
alpha, and global `alpha` multiplies it.
...
coords <- rbind(A = c(0, 0, 0), B = c(1, 1, 0),
                C = c(2, 0, 1), D = c(0, 2, 1))
graph <- list(vertices = vertices, edges = edges)
```

```{r embedded-graph, eval=have.rgl}
plot3D.graph(graph, X = coords, edge.col = "gray50", edge.width = 2,
...
coordinates follow vertex order. Vertex values and groups always follow graph
vertex order, and layer row indices refer to that order after alignment.
Edge colors and widths follow normalized edge order, visible in
`attr(widget, "ivue")$graph$edges`; matrix/list inputs use one copy of each
