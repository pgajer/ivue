...
Point-cloud `values`, `groups`, colors, and highlight masks follow **row
position**, even when annotation vectors have names. Match external annotations
to coordinate IDs yourself and check uniqueness and completeness first, as in
[the recipes](example-data.html#construct-a-point-cloud). Missing numerical
...
ivue registers **0 S3 methods** and re-exports no functions. Dots in names such
...
`attr(widget, "ivue")` records coordinates, row IDs, mapped colors, highlight,
...
`center` requires a supplied diverging palette or `color.map`, places the
center at the palette midpoint, and makes automatic limits symmetric about
it. With explicit limits, the center must lie strictly inside them. A constant
...
vectors use first-occurrence order. Named colors must cover the reference
...
function runs at mapping time and must return one color per supplied value;
its external state is the caller's responsibility. Supply it or `palette`,
not both.
...
contains `X`, `row.ids`, `colors`, `highlight`, and `draw.ids`; `args` supplies
...
R-side camera metadata or synchronize another widget.
...
export. Neither export operation captures later browser interaction.
